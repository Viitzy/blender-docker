...
def bake_diffuse_color(objects):
    ...
    bpy.context.preferences.addons["cycles"].preferences.compute_device_type = (
        "CUDA"  # or 'OPTIX' for RTX cards, 'HIP' for AMD, 'ONEAPI' for Intel
    )
...
road_line_points = [(x, y, z - 0.13) for (x, y, z) in road_line_points]
...
# New main block to run vehicle_road_3d.py using a CSV file for elevation colors
if __name__ == "__main__":
    import argparse
    import csv

    parser = argparse.ArgumentParser(
        description="Run vehicle_road_3d using a CSV file for elevation colors"
    )
    parser.add_argument(
        "--csv",
        type=str,
        default="front_cardinal_utm_elevation_colors_test_-20.16577547484259_-43.95320188043381.csv",
        help="Path to the CSV file",
    )
    args = parser.parse_args()

    # Read CSV data using the existing function
    csv_data = read_csv_data(args.csv)

    # Extract (x, y) coordinates and z values from CSV rows (skipping header rows)
    points = []
    zs = []
    for row in csv_data:
        try:
    ...
    # Determine the minimum z value from CSV data
    min_z = min(zs) if zs else 0

    # Project the hull points to the XY plane using the minimum z as reference

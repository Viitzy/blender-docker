...
def get_top_left_world_pixel(
    center_lat, center_lon, zoom, scale, image_width, image_height
):
    """
    Calcula as coordenadas de pixel do canto superior esquerdo da imagem no mundo.
    """
    center_x, center_y = latlon_to_world_pixel(
        center_lat, center_lon, zoom, scale
    )
...
def pixel_to_latlon(
    ...
    top_left_x, top_left_y = get_top_left_world_pixel(
        center_lat, center_lon, zoom, scale, image_width, image_height
    )
...
def lat_lon_to_pixel_normalized(
    ...
    """
    Converte coordenadas geográficas (latitude, longitude) para coordenadas de pixel normalizadas (0-1).
    Esta função é o processo inverso de pixel_to_latlon.
    ...
    Returns:
        (x, y): Tupla com coordenadas normalizadas (0-1) do pixel na imagem.
    """
    # Primeiro, converte lat/lon para coordenadas de pixel no mundo
    world_x, world_y = latlon_to_world_pixel(lat, lon, zoom, scale)
    ...
    # Calcula as coordenadas de pixel na imagem
    pixel_x = world_x - top_left_x
    pixel_y = world_y - top_left_y
